...
class ZerodhaOrderManager(BaseOrderManager):
  ...
  def modifyOrderToMarket(self, order):
    logging.info('%s: Going to modify order with params %s', self.broker, str(order))
    kite = self.brokerHandle
    try:
      orderId = kite.modify_order(
        variety=kite.VARIETY_REGULAR,
      ...
      logging.info('%s Order will update to MARKET for orderId = %s', self.broker, orderId)
      order.lastOrderUpdateTimestamp = Utils.getEpoch()
      return order
    except Exception as e:
      if ('Order cannot be modified as it is being processed' in str(e)):
        logging.info('%s Order Will update in next process: %s', self.broker, str(e))
        return True
      else:
  ...
  def cancelOrder(self, order):
    ...
    try:
      ...
      if ('Order cannot be cancelled as it is being processed' in str(e)):
        logging.info('%s Order Will update in next process: %s', self.broker, str(e))
        return True
      else:
