...
class ShortStraddleBNF(BaseStrategy):
  ...
  def __init__(self):
    ...
    self.startTimestamp = Utils.getTimeOfToDay(9, 59, 59) # When to start the strategy. Default is Market start time
    ...
    self.leverage = 0
    self.maxTradesPerDay = 4 # (1 CE + 1 PE) Max number of trades per day under this strategy
  ...
  def process(self):
    ...
    # Get current market price of BANK NIFTY Future
    ...
    if lastTradedPrice == None:
      logging.error('%s: Could not get quote for %s', self.getName(), bankNiftySymbol)
      return

    ATMStrike = Utils.getNearestStrikePrice(lastTradedPrice, 100)
    logging.info('%s: Nifty CMP = %f, ATMStrike = %d', self.getName(), lastTradedPrice, ATMStrike)

    #Margin Order
    MARGINPESymbol = Utils.prepareWeeklyOptionsSymbol("BANKNIFTY", 31000, 'PE');
    MARGINCESymbol = Utils.prepareWeeklyOptionsSymbol("BANKNIFTY", 40000, 'CE');
    self.generateTrades(MARGINCESymbol,MARGINPESymbol, True);
  ...
  def generateTrades(self, ATMCESymbol, ATMPESymbol, isMargin=False):
    ...
    self.generateTrade(ATMCESymbol, numLots, quoteATMCESymbol.lastTradedPrice, isMargin)
    self.generateTrade(ATMPESymbol, numLots, quoteATMPESymbol.lastTradedPrice, isMargin)
    logging.info('%s: Trades generated.', self.getName())

  def generateTrade(self, optionSymbol, numLots, lastTradedPrice, isMargin=False):
    trade = Trade(optionSymbol)
    trade.strategy = self.getName()
    trade.isOptions = True
    trade.direction = Direction.SHORT if isMargin==False else Direction.LONG  # Always short here as option selling only
    ...
    trade.stopLoss = 0 if isMargin == True else Utils.roundToNSEPrice(trade.requestedEntry + trade.requestedEntry * self.slPercentage / 100)
