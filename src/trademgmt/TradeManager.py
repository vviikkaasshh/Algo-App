...
class TradeManager:
    ...
    def run():
        try:
            ...
            if os.path.exists(TradeManager.intradayTradesDir) == False:
                logging.info(
                    'TradeManager: Intraday Trades Directory %s does not exist. Hence going to create.', TradeManager.intradayTradesDir)
            ...
            TradeManager.ticker.registerListener(TradeManager.TrailSlListener)
            TradeManager.ticker.registerListener(TradeManager.BookProfit)

            # sleep for 2 seconds for ticker connection establishment
            time.sleep(2)

            # Load all trades from json files to app memory
            TradeManager.sendUrlToTelegram()
            TradeManager.loadAllTradesFromFile()
            TradeManager.ticker.registerSymbols(
                ['NIFTY 50', 'NIFTY BANK'])  # Subscribe Index
            ...
            while True:
                ...
                try:
                    ...
                    Logger.getInstance().log("Exception in TradeManager Main thread" +
                                             str(e), Condition.MESSAGE)
                ...
                time.sleep(TradeManager.getSleepSeconds())
                logging.info('TradeManager: Main thread woke up..')
        finally:
            logging.info('TradeManager: Exit from finally..')
    ...
    def tickerListener(tick):
        try:
            ...
            logging.info(
                'TradeManager: tickerListener Failed: Error => %s', str(e))
            Logger.getInstance().log('TradeManager: tickerListener Failed' +
                                     str(e), Condition.MESSAGE)

    @staticmethod
    def TrailSlListener(tick):
        try:
            for trade in TradeManager.trades:
                if trade.tradeState == TradeState.ACTIVE and trade.slOrder != None:
                    TradeManager.checkAndUpdateTrailSL(trade)
        except Exception as e:
            Logger.getInstance().log('TradeManager: TrailSlListener Failed' +
                                     str(e), Condition.MESSAGE)
            logging.info(
                'TradeManager: TrailSlListener Failed: Error => %s', str(e))
        ...
        try:
            for trade in TradeManager.trades:
                if trade.tradeState == TradeState.ACTIVE:
                    strategyInstance = TradeManager.strategyToInstanceMap[trade.strategy]
                    strategyInstance.SquareOffOnProfit()
        except Exception as e:
            Logger.getInstance().log('TradeManager: BookProfit Failed' + str(e), Condition.MESSAGE)
            logging.info(
                'TradeManager: BookProfit Failed: Error => %s', str(e))
    ...
    def executeTrade(trade):
        ...
        try:
            ...
            Logger.getInstance().log('TradeManager: Execute trade failed' +
                                     str(e), Condition.MESSAGE)
            logging.error(
    ...
    def getSleepSeconds():
        try:
            sleepSeconds = 30
            sortedTrades = sorted(TradeManager.trades,
                                  key=lambda x: x.intradaySquareOffTimestamp)
            for trade in sortedTrades:
                if trade.tradeState == TradeState.ACTIVE:
                    if trade.intradaySquareOffTimestamp != None and abs(trade.intradaySquareOffTimestamp - Utils.getEpoch(datetime.now())) <= sleepSeconds:
                        return abs(trade.intradaySquareOffTimestamp - Utils.getEpoch(datetime.now()))
            return sleepSeconds
        except Exception as e:
            Logger.getInstance().log('TradeManager: Failed to find Sleep seconds' +
                                     str(e), Condition.MESSAGE)
            logging.error(
                'TradeManager: Failed to find Sleep seconds: Error => %s', str(e))
            return sleepSeconds
    ...
    def trackSLOrder(trade):
        ...
        if trade.slOrder == None:
            ...
            if trade.slOrder.orderStatus == OrderStatus.COMPLETE:
                ...
                TradeManager.cancelTargetOrder(trade)
            elif trade.slOrder.orderStatus == OrderStatus.OPEN:
                logging.info('SL order triggered but not executed')
                Logger.getInstance().message("SL order triggered but not executed in %s Symbol %s",
                                             trade.strategy, trade.tradingSymbol)
    ...
    def checkAndUpdateTrailSL(trade):
        if trade.slOrder.orderModifiedCount > 20:
            logging.info('TradeManager: Trade modification exceeded for Trade Id %s orderId %s',
                         trade.tradeID, trade.slOrder.orderId)
            if TradeManager.cancelSLOrder(trade):
                trade.slOrder = None
            return
        ...
        if updateSL == True:
            ...
            try:
                ...
                trade.slOrder.orderModifiedCount += 1
                logging.info('TradeManager: Trail SL: Successfully modified stopLoss from %f to %f for tradeID %s for %f times',
                             oldSL, newTrailSL, trade.tradeID, trade.slOrder.orderModifiedCount)
            except Exception as e:
                Logger.getInstance().log("TradeManager: Failed to modify SL order", Condition.MESSAGE)
    ...
    def placeSLOrder(trade):
        ...
        variance = TradeManager.GetVarianceForSlOrder(trade)
        oip.price = oip.triggerPrice - \
            variance if trade.direction == Direction.LONG else oip.triggerPrice + variance
        oip.qty = trade.qty
        if trade.isFutures == True or trade.isOptions == True:
            oip.isFnO = True
        try:
            ...
            Logger.getInstance().log("TradeManager: Failed to place SL order", Condition.MESSAGE)
            logging.error(
    ...
    def placeTargetOrder(trade, isMarketOrder=False):
        ...
        try:
            ...
            Logger.getInstance().log("TradeManager: Failed to place Target order", Condition.MESSAGE)
            logging.error(
    ...
    def cancelEntryOrder(trade):
        ...
        try:
            ...
            Logger.getInstance().log("TradeManager: Failed to place Entry order", Condition.MESSAGE)
    ...
    def cancelSLOrder(trade):
        ...
        if trade.slOrder.orderStatus == OrderStatus.CANCELLED:
            return
        try:
            InException = TradeManager.getOrderManager().cancelOrder(trade.slOrder)
            if type(InException) == bool and InException:
                TradeManager.setTradeToCompleted(
                    trade, TradeManager.getLastTradedPrice(trade.tradingSymbol), "In-Exception")
        except Exception as e:
            Logger.getInstance().log("TradeManager: Failed to cancel SL order", Condition.MESSAGE)
    ...
    def squareOffTrade(trade, reason=TradeExitReason.SQUARE_OFF):
        ...
        if trade.targetOrder != None:
            ...
            logging.info('TradeManager: changing target order %s to MARKET to exit position for tradeID %s',
                         trade.targetOrder.orderId, trade.tradeID)
            InException = TradeManager.getOrderManager().modifyOrderToMarket(trade.targetOrder)
            if InException:
                TradeManager.setTradeToCompleted(
                    trade, trade.targetOrder.averagePrice, "In-Exception")
        elif trade.tradeState == TradeState.ACTIVE:
    ...
    def convertJSONToOrder(jsonData):
        ...
        order.orderModifiedCount = jsonData['orderModifiedCount']
        return order
    ...
    @staticmethod
    def sendUrlToTelegram():
        try:
            import urllib.request
            external_ip = urllib.request.urlopen(
                'https://api.ipify.org').read().decode('utf8')
            Logger.getInstance().message(
                f'Url for PnL: https://{external_ip}/pnlStatus')
        except Exception as e:
            logging.info('TradeManager: Get Public IP failed %s', {str(e)})
    
    @staticmethod
    def GetVarianceForSlOrder(trade):
        if "Sniper" in trade.strategy:
            return 0.5
        return 0.05
